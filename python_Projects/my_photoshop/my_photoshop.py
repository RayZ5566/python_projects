...
def get_best_pixel(pixels):
    ...
    for pixel in pixels:
        ...
        best_pixel[pixel] = distance
    best_pixel = min(best_pixel.items(), key=lambda t: t[1])
    return best_pixel[0]
...
def solve(images):
    ...
    for x in range(result.width):
        for y in range(result.height):
            ...
            for image in images:
                rgb_list.append(image.get_pixel(x, y))
            result_p.red = get_best_pixel(rgb_list).red
            result_p.green = get_best_pixel(rgb_list).green
            result_p.blue = get_best_pixel(rgb_list).blue
    ######## YOUR CODE ENDS HERE ###########
